...
-->
<template>
  <div v-if="displayType === 1" v-loading="loading" element-loading-background="rgba(0, 0, 0, 0.5)">
    <Panel>
          ...
          >
            <ItemGroup>
              <QuickStartButton style="height: 120px">
                ...
                <template #info>
                  <p class="sub-title-info">{{ item.subTitle }}</p>
                </template>
              </QuickStartButton>
            </ItemGroup>

            <div v-if="item.extra && isCN()" style="text-align: center">
              <el-link type="primary" @click="item.fn(item.value)">{{ item.extra.title }}</el-link>
            </div>
          </el-col>
        </el-row>
      </div>
